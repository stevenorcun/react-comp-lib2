...
const DrawerNavigation = ({
  ...
  return (
    <>
      <div className={cx(styles.navigation, commons.PrettyScroll, className, 'drawer-navigation')}>
        {menus.map(
          (menu, index) => ((index < 2 && !fullMenu) || fullMenu) && (
          <div
            key={`${menu.key}-tab`}
        ...
        {!fullMenu && menus.length > 3 && (
          ...
          >
            {menus.length - 2}
            {' '}
            <IconDoubleArrow style={{ marginLeft: 5 }} />
          </div>
        )}
      </div>
    </>
