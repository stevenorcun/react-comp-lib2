...
const Drawer = ({
  ...
  useEffect(() => {
    if (drawerRef.current?.style) {
      drawerRef.current.style[position] = isCollapsed ? `${-fullWidth - 1}px` : '0';
    }
  }, [isCollapsed]);
  ...
  const onMouseDown = (e) => {
    ...
    if (isResizable && e.button === 0 && borderRef.current?.contains(e.target)) {
      setResizing(true);
      setMouseDown(true);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className={cx(styles.Drawer__Body, 'drawer-drawerBody')}>
        ...
        />
        <div className={cx(commons.PrettyScroll, styles.Drawer__Tab, 'drawer-drawerTab')}>
          {menus.map((menu) => (activeMenu === menu.key ? (
